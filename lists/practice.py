...
def print_list(items):
    ...
    # Iterating through the item list and printing each item
    for item in items:
        print item
...
def halvesies(numbers):
    ...
    the half::
        ...
        [0.5, 2.5]
    """
    half_list = [float(num)/2.0 for num in numbers]
...
def average(numbers):
    ...
    if len(numbers) == 0:
        ...
        avg_num = float(total) / float(len(numbers))
        return avg_num
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        ...
        # Iterating through all the letters
            found_index = False
            while i < len(word) and found_index == False:
